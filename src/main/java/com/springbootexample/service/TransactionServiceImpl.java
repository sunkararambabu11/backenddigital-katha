...
public class TransactionServiceImpl implements TransactionService {
	...
	public void deleteTransaction(Long id, Long userId) {
		...
		Transaction txn = txnRepo.findById(id).orElse(null);

		if (txn == null) {
			System.out.println("Transaction not found in DB");
			throw new RuntimeException("Transaction not found");
		}

		System.out.println("CustomerId: " + txn.getCustomerId());

		Customer customer = customerRepo.findById(txn.getCustomerId())
				.orElseThrow(() -> new RuntimeException("Customer not found"));

		System.out.println("JWT User: " + userId);
		System.out.println("Customer User: " + customer.getUserId());
